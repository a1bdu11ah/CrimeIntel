  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        ...
        <ArrowUpRight
          size={16}
          className="text-slate-500 transition-all group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-navy-900"
        />
      </div>

      <div className="mt-6">
        <p className="font-heading text-4xl font-bold leading-none text-navy-900">
...
const activityIcons = {
  fir: <FileText size={14} className="text-sky-700" />,
  arrest: <UserX size={14} className="text-rose-700" />,
  evidence: <Package size={14} className="text-amber-700" />,
  closed: <FileCheck size={14} className="text-emerald-700" />,
  personnel: <Users size={14} className="text-violet-700" />,
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="attention-banner">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-navy-900">3 cases require immediate attention</p>
            <p className="text-xs text-slate-500">Review pending FIRs and investigation updates before end of day.</p>
          </div>
        </div>
        <button onClick={() => navigate("/case/progress")} className="hidden text-xs font-semibold text-amber-700 hover:text-amber-700 sm:block">
          Review cases →
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard label="Criminal Records" value={totalCriminals} icon={UserX} accent="stat-navy" change="4" helper="Profiles in database" to="/criminal/view" />
        <StatCard label="Active Cases" value={activeCases} icon={TrendingUp} accent="stat-sky" helper="Currently in progress" to="/case/progress" />
        <StatCard label="Personnel" value={personnelCount} icon={Users} accent="stat-steel" change="2" helper="Registered officers" to="/personnel/view" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <section className="card xl:col-span-2">
          <div className="mb-5 flex items-center justify-between gap-3">
            <div>
              ...
              <h3 className="mt-1 font-heading text-xl font-semibold text-navy-900">Recent Activity</h3>
            </div>
            ...
            {recentActivity.map((item) => (
              <div key={item.id} className="activity-row">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-navy-900">{item.action}</p>
                  <p className="mt-1 truncate text-xs text-slate-500">{item.detail}</p>
                </div>
                <span className="hidden flex-shrink-0 text-[11px] text-slate-500 sm:block">{item.time}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="card">
          <div className="mb-5">
            ...
            <h3 className="mt-1 font-heading text-xl font-semibold text-navy-900">Case Distribution</h3>
          </div>
